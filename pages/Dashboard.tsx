                                        ...
                                        {enrollments.filter(e => e.courseId === selectedCourseId && e.paymentStatus === 'PAID').map(student => {
                                            ...
                                            return (
                                                <tr key={student.id} className={`hover:bg-slate-50 transition-colors ${checkedInToday ? 'bg-green-50/30' : ''}`}>
                                                    ...
                                                    <td className="p-5"><div className="flex items-center gap-2 mb-1"><span className="text-sm font-bold text-slate-700">{attendedCount}/{totalSessions} ครั้ง {/* เพิ่มคำว่า ครั้ง ตรงนี้ */}</span></div<div className="w-32 bg-slate-200 rounded-full h-2"><div className="bg-ocean-500 h-2 rounded-full" style={{ width: `${Math.min((attendedCount/totalSessions)*100, 100)}%` }}></div></div></td>
